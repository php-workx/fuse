...
| `FUNC-17` / `TECH-1.2` / `testplan.md` §6 | Performance claims and low-friction hot path are proven | `functional.md` §17, `technical_v1.1.md` §1.2, `specs/testplan.md` §6 | `partial` | `specs/testplan.md`, `README.md`, `internal/releasecheck/releasecheck_test.go`, `scripts/run-release-checks.sh`, `docs/release/2026-03-12-perf-compat-smoke.md` | A release-check harness now exists and the current machine comfortably meets warm/cold shell and MCP hot-path targets, but `PERF-003` still fails on large unmatched inputs and several compatibility slices remain unmeasured. |
