...
func TestUnconditionalSafeCmd_MultiWord(t *testing.T) {
	tests := []struct {
		...
		{"npm test with args", "npm test -- --runInBand", true},
		{"npm run lint", "npm run lint", true},
		{"npm run test", "npm run test", true},
		{"npm run check", "npm run check", true},
		{"npm run validate", "npm run validate", true},
		{"npm run verify", "npm run verify -- --ci", true},
		{"npx jest", "npx jest --coverage", true},
		{"yarn test", "yarn test", true},
		{"yarn lint", "yarn lint --max-warnings=0", true},
		{"yarn check", "yarn check", true},
		{"yarn typecheck", "yarn typecheck", true},
		{"yarn run validate", "yarn run validate", true},
		{"yarn run test", "yarn run test", true},
		{"pnpm test", "pnpm test", true},
		{"pnpm lint", "pnpm lint", true},
		{"pnpm check", "pnpm check", true},
		{"pnpm typecheck", "pnpm typecheck", true},
		{"pnpm run verify", "pnpm run verify", true},
		{"pnpm run lint", "pnpm run lint", true},
		...
		{"npm test update snapshots", "npm test -- --updateSnapshot", false},
		{"npm run validate force", "npm run validate -- --force", false},
		{"npm run build", "npm run build", false},
		{"npm run deploy", "npm run deploy", false},
		{"yarn lint fix", "yarn lint --fix", false},
		{"yarn typecheck write", "yarn typecheck --write", false},
		{"yarn build", "yarn build", false},
		{"yarn deploy", "yarn deploy", false},
		{"pnpm lint fix", "pnpm lint --fix", false},
		{"pnpm typecheck update", "pnpm typecheck --update", false},
		{"pnpm build", "pnpm build", false},
		{"pnpm run deploy", "pnpm run deploy", false},
		{"pnpm publish", "pnpm publish", false},
