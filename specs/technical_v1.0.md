...
- `mattn/go-shellwords` for shell tokenization (same library used by SLB).
...
**Performance note:** Pure-Go SQLite (`modernc.org/sqlite`) is ~3-5x slower than CGo SQLite for cold start. Each hook invocation opens a new connection, sets WAL mode, and queries. Mitigation strategies (validate with prototype benchmarking):
- **Lazy SQLite open:** Skip database access for SAFE/BLOCKED classifications that don't need approval state. Only open the database when an APPROVAL decision is reached or when logging events.
- **Pre-compiled regex:** All regex patterns compiled at package init, not per-invocation.
- **Connection reuse (future):** If latency exceeds targets, consider a long-running daemon with Unix socket for hot-path queries.
...
fuse/
  ...
  internal/
    ...
    core/
      ...
      normalize_test.go
      compound.go        # compound command splitting (;, &&, ||, |, \n)
...
| `github.com/mattn/go-shellwords` | Shell tokenization | latest stable |
...
- `timeout` is in **seconds** (not milliseconds). 30 seconds allows for TUI approval prompts.
...
On denial (exit 2), fuse writes a **plain text** reason to stderr. Claude Code sends this text to Claude as an error message, so Claude can adjust its behavior:

```
fuse: BLOCKED — rm -rf / is classified as catastrophic filesystem destruction
```

```
fuse: DENIED — user denied approval for terraform destroy PaymentsStack
```

Stderr is plain text, not JSON. On exit 0 (allow), fuse must not write to stdout — stdout output is processed by Claude Code and would interfere. Diagnostic logging goes to stderr only via `log/slog`.
...
6. If APPROVAL: render TUI prompt on `/dev/tty`, exit 0 on approve, exit 2 on deny.
7. If BLOCKED: write reason to stderr, exit 2.
8. Log event to SQLite (lazy — skip if SAFE and database not yet opened).

### 4.2 `fuse run -- <command>`

Manual usage and testing.

1. Parse command from argv (everything after `--`).
2. Run classification pipeline (§5).
3. If APPROVAL: render TUI prompt on terminal.
4. If allowed/approved: execute via `sh -c` subprocess, stream stdout/stderr, return exit code.
5. If denied/blocked: print reason, exit 1.
...
func SplitCompoundCommand(displayNorm string) []string {
    ...
    // Respect quoting: do not split inside "..." or '...'
    // Use mattn/go-shellwords or equivalent for proper quote-aware splitting
...
func ClassificationNormalize(subCommand string) ClassifiedCommand {
    ...
    //    using mattn/go-shellwords for proper quote handling.
...
| Command pattern | Referenced file | Regex |
|---|---|---|
| `python file.py` / `python3 file.py` | `file.py` | `\bpython[23]?\s+(?!-[cmeWvV])(\S+\.py)\b` |
| `node file.js` / `node file.ts` | `file.js` / `file.ts` | `\bnode\s+(?!-[ep])(\S+\.[jt]s)\b` |
| `bash script.sh` / `sh script.sh` | `script.sh` | `\b(ba)?sh\s+(?!-c)(\S+\.sh)\b` |
| `./script.sh` / `/path/to/script` | the path | `^[./]\S+` (then check if file exists and is executable) |
| `ruby script.rb` / `perl script.pl` | the script | `\b(ruby\|perl)\s+(\S+\.(rb\|pl))\b` |

File path is resolved relative to the command's cwd. Symlinks are resolved to their canonical path via `filepath.EvalSymlinks()` before inspection — hash and inspect the canonical target, not the symlink. If the file does not exist, skip inspection (no error — it may be created at runtime).
...
| `builtin:git:restore-staged` | `\bgit\s+restore\s+(?!.*--staged)\S` | CAUTION | May discard working tree changes |
...
The `inspect_file` action triggers file inspection (§7). The final decision comes from the inspection results, not from this rule alone. If inspection finds no signals, the command is SAFE.
...
func InspectFile(path string, maxBytes int64) (*FileInspection, error) {
    ...
    // 2. Stat file. If > maxBytes, set truncated = true.
    // 3. Read file content (up to maxBytes).
    // 4. Compute SHA-256 hash of full content (not truncated).
    // 5. Determine file type from extension.
    // 6. If file type has a scanner (sh, py, js, ts): dispatch to scanner.
    //    If file type is detected but unsupported (rb, pl, go, etc.):
    //    set Risk = "caution" (cannot analyze, but could contain anything).
    // 7. If truncated AND no signals found in inspected portion:
    //    set Risk = "approval" (mandatory — truncated files are not fully analyzed).
    // 8. Return FileInspection with signals.
}
```
...
Decision keys use length-prefixed fields to avoid null-byte ambiguity:
...
--- fuse: approval required ---
  ...
  Command:  terraform destroy PaymentsStack
...
- Timeout: 5 minutes of no input -> auto-deny, log timeout event.
- If `/dev/tty` cannot be opened (e.g., no terminal): auto-deny with error message.
- Wrap all TUI code in panic recovery. On panic: log error to stderr, auto-deny.
...
func Execute(ctx context.Context, command string, cwd string) (int, error) {
    ...
    cmd.Env = os.Environ()  // inherit_all (v1 default)
...
fuse speaks JSON-RPC 2.0 over stdio in both directions. It is a transparent proxy that only intercepts `tools/call` requests.
...
**Reuse directly:**
- Command normalization pipeline design (`internal/core/normalize.go`): wrapper stripping, compound splitting logic. Port the approach, not the code verbatim.
...
**Reuse:**
- Detection pattern corpus: 49+ packs. The regex patterns are plain strings — port directly. Section §6.3 contains the v1 subset already ported.
...
**Mitigation:** File inspection at classification time catches the common case (agent writes a destructive script then immediately runs it). For stronger guarantees, use `fuse run` mode where fuse controls both classification and execution atomically. In `fuse run` mode, consider re-verifying the file hash immediately before `exec.Command`.
